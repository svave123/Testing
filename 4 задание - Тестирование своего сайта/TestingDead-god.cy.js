describe("Тестирование функции поиска на сайте dead-god.ru", () => {
    ...
    cy.go('back');
         ...
         it('Тест6: Проверка путей у ссылок ', () => {
            cy.get('.list[data-item-type="item"]').should('be.visible');
            cy.get('.list[data-item-type="item"]').invoke('hide');
            cy.get('.list[data-item-type="item"]').should('not.be.visible');
            });

         it('Тест7: Проверка фильтров', () => {
      cy.get('.filter__button').click();
      cy.get('label[for="color_7"]').click();
      cy.get('body').should('have.css', 'background-color', 'rgb(67, 45, 40)');
      cy.get('label[for="color_2"]').click();
      cy.get('body').should('have.css', 'background-color', 'rgb(40, 40, 42)');


      });
    it('Тест8: Проверка фильтров', () => {
        cy.get('.filter__button').click();
        cy.get('label[for="spacing"]').should('have.css', 'color', 'rgb(204, 102, 102)');
        cy.get('label[for="spacing"]').click();
        });
